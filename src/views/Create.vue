    ...
    <div class="p-8 flex items-start bg-light-grey rounded-md shadow-lg">
      <form @submit.prevent="createWorkout" class="flex flex-col gap-y-5 w-full">
        ...
        <div class="flex flex-col">
          ...
          <input type="text" required class="p-2 text-grey-500 focus:outline-none" id="workout-name" v-model="workoutName">
        </div>

        <div class="flex flex-col">
          <label for="workout-type" class="mb-1 text-sm text-at-light-green">Workout Type</label>
          <select @change="workoutChange" id="workout-type" class="p-2 text-grey-500 focus:outline-none" required v-model="workoutType">
        ...
        <div v-if="workoutType === 'strength'" class="flex flex-col gap-y-4">
          ...
          <button @click="addExercise" type="button" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green duration-200 border-solid border-2 border-transparent
              hover:border-at-light-green hover:bg-white hover:text-at-light-green">Add Exercise
          </button>
        </div>
        ...
        <div v-if="workoutType === 'cardio'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises" :key="index">
            <div class="flex flex-col md:w-1/3">
              <label for="cardiot-type" class="mb-1 text-sm text-at-light-green">Exercise</label>
        ...
        <button type="submit" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green duration-200 border-solid border-2 border-transparent
              hover:border-at-light-green hover:bg-white hover:text-at-light-green">Record Workout
        </button>
  ...
  setup() {
    ...
    const createWorkout = async () => {
      try{
        ...
        errorMsg.value = `Error: ${error.messsage}`;
        setTimeout(() => {
          errorMsg.value = false;
        }, 5000)
      }
