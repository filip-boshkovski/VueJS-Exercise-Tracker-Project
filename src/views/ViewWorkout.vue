  ...
  <div class="max-w-screen-sm mx-auto px-4 py-10">
    ...
    <div v-if="dataLoaded">
      <div class="flex flex-col items-center p-8 rounded-md shadow-md bg-light-grey relative">
        ...
        <span class="mt-6 py-2 px-5 text-xs text-white bg-at-light-green rounded-lg shadow-md">
          {{ data.workoutType }}
        </span>
        ...
        <div v-if="data.workoutType === 'strength'" class="flex flex-col gap-y-4 w-full">
          <div class="flex flex-col gap-x-6 gap-y-2 relative sm:flex-row" v-for="(item, index) in data.exercises" :key="index">
            <div class="flex flex-2 flex-col md:w-1/3">
              ...
              <input id="exercise-name" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.exercise">
              <p v-else>{{ item.exercise }}</p>
            </div>

            <div class="flex flex-1 flex-col">
              <label for="sets" class="mb-1 text-sm text-at-light-green">Sets</label>
              <input id="sets" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.sets">
              <p v-else>{{ item.sets }}</p>
            </div>

            <div class="flex flex-1 flex-col">
              <label for="reps" class="mb-1 text-sm text-at-light-green">Reps</label>
              <input id="reps" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.reps">
              <p v-else>{{ item.reps }}</p>
            </div>

            <div class="flex flex-1 flex-col">
              <label for="weight" class="mb-1 text-sm text-at-light-green">Weight (LB's)</label>
              <input id="weight" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.weight">
          ...
          <button @click="addExercise" v-if="edit" type="button" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green duration-200 border-solid border-2 border-transparent
        ...
        <div v-else class="flex flex-col gap-y-4 w-full">
          <div class="flex flex-col gap-x-6 gap-y-2 relative sm:flex-row" v-for="(item, index) in data.exercises" :key="index">
            <div class="flex flex-2 flex-col md:w-1/3">
              ...
              <select id="cardioType" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.cardioType">
            ...
            <div class="flex flex-1 flex-col">
              ...
              <input id="distance" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.distance">
              <p v-else>{{ item.distance }}</p>
            </div>

            <div class="flex flex-1 flex-col">
              <label for="duration" class="mb-1 text-sm text-at-light-green">Duration</label>
              <input id="duration" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.duration">
              <p v-else>{{ item.duration }}</p>
            </div>

            <div class="flex flex-1 flex-col">
              <label for="pace" class="mb-1 text-sm text-at-light-green">Pace</label>
              <input id="pace" v-if="edit" class="p-2 w-full text-grat-500 focus:outline-none" type="text" v-model="item.pace">
      ...
      <button type="button" @click="update" v-if="edit" class="mt-10 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green duration-200 border-solid border-2 border-transparent
